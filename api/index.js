...
// Aqui está o seu System Prompt completo
...
module.exports = async (req, res) => {
  if (req.method === 'POST') {
    ...
    if (message && message.text) {
      ...
      try {
        // Usando a versão v1beta para melhor suporte a instruções de sistema
        ...
        const response = await axios.post(geminiUrl, {
          contents: [
            ...
            {
              role: "model",
              parts: [{ text: "I understand perfectly. I am ready to be your witty, warm, and creative English-speaking collaborator. How can we dive in today?" }]
            },
            {
              role: "user",
              parts: [{ text: userText }]
        ...
        // Resposta de erro seguindo a personalidade
        await sendMessage(chatId, "I've hit a small snag in my creative gears. Let's try that again in a moment, shall we?");
      }
    }
    return res.status(200).send('OK');
  }
  res.status(200).send('Bot is Alive with Full Personality!');
};const axios = require('axios');
      ...
      try {
        ...
        const response = await axios.post(geminiUrl, {
          contents: [
            ...
            { role: "model", parts: [{ text: "Understood. I will be your creative, witty assistant and speak only in English from now on." }] },
        ...
        // Envia uma mensagem amigável em caso de erro
        await sendMessage(chatId, "I'm having a brief moment of static in my creative modules. Could you try again?");
      }
    }
    return res.status(200).send('OK');
  }
  res.status(200).send('Bot is Alive and Ready with the right Gemini Key!');
};
